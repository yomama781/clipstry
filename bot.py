...
class Clipstry(discord.Client):
    ...
    async def setup_hook(self):
        ...
        register_extra_commands(
            self.tree, self.db, CAMPAIGN_MANAGER_ROLE, has_role,
            fetch_post_views, detect_platform_from_url, normalize_handle,
            APIFY_TOKEN, _apify_run,
        )
        try:
...
def register_commands(tree: app_commands.CommandTree, db):
    ...
    async def create_campaign_cmd(
        ...
        camp = {
            ...
            "created_at": datetime.now(timezone.utc).isoformat(),
            "ended_at": None,
            "allowed_tiers": ["Tier 1", "Tier 2", "Tier 3"] if allowed_tiers.lower() == "all" else [t.splitHere's the full bot.py:

```python
"""
Clipstry Discord Bot — single-file edition.

Required env vars (set these in your hosting panel, NOT in the file):
  DISCORD_BOT_TOKEN   - Bot token from Discord Developer Portal
  MONGO_URL           - mongodb://... or mongodb+srv://...
  DB_NAME             - e.g. viewtracker_db
  APIFY_API_TOKEN     - (optional) for reliable Instagram/X/TikTok scraping

Required packages: discord.py httpx beautifulsoup4 lxml motor python-dotenv
"""
import os
import re
import json
...
def register_commands(tree: app_commands.CommandTree, db):
    ...
    @tree.command(name="setup-channel", description="Set this channel as the clip review channel (Admin only)")
    @app_commands.default_permissions(administrator=True)
    async def setup_channel_cmd(interaction: discord.Interaction):
        await db.clip_config.update_one(
            {"guild_id": str(interaction.guild_id)},
            {"$set": {"channel_id": interaction.channel_id}},
            upsert=True,
        )
        await interaction.response.send_message(
            f"✅ This channel is now the **clip review channel**! Clips submitted via the panel will appear here for admin accept/deny.",
            ephemeral=True
        )

    @tree.command(name="mystats", description="Check your accepted clip count")
    async def mystats_cmd(interaction: discord.Interaction):
        uid = str(interaction.user.id)
        doc = await db.clip_stats.find_one({"discord_id": uid})
        if not doc:
            await interaction.response.send_message(
                "📊 **No clips submitted yet!** Use the Submit Clip button to submit one.",
                ephemeral=True
            )
            return
        accepted = doc.get("accepted", 0)
        denied = doc.get("denied", 0)
        total = accepted + denied
        embed = discord.Embed(
            title=f"📊 {interaction.user.display_name}'s Clip Stats",
    ...
    @tree.command(name="leaderboard", description="Show top clip submitters")
    async def leaderboard_cmd(interaction: discord.Interaction):
        top = await db.clip_stats.find().sort("accepted", -1).limit(10).to_list(10)
        if not top:
            await interaction.response.send_message(
                "📭 **No clips have been accepted yet!** Be the first!",
                ephemeral=True
            )
            return
        embed = discord.Embed(
            title="🏆 Clip Leaderboard",
            description="Top 10 users by accepted clips",
            color=discord.Color.gold()
        )
        medals = ["🥇", "🥈", "🥉"]
        for i, doc in enumerate(top):
            uid = doc.get("discord_id", "0")
            ...
            embed.add_field(
                name=f"{rank} {name}",
                value=f"✅ {accepted} accepted | ❌ {denied} denied",
                inline=False
            )
